// ported from https://raw.githubusercontent.com/alextanhongpin/node-bandit/master/src/utils/math2.js

export const sum = (arr: number[]) => {
  return arr.reduce((total, val) => total + val, 0)
}

export const maxIndex = (arr: number[]) => {
  return arr.indexOf(Math.max(...arr))
}

export function rbeta (alpha: number, beta: number) {
  const alphaGamma = rgamma(alpha, 1)
  return alphaGamma / (alphaGamma + rgamma(beta, 1))
}

export function rgamma (alpha: number, beta: number) {
